...
- **Always use the design system for ALL UI styling.** Never hardcode colors, font sizes, font families, spacing, or radii in screens or components. Use `T.*` tokens from `src/theme/theme.ts` for colors (`T.bg.*`, `T.accent.*`, `T.text.*`, `T.status.*`), font sizes (`T.font.*`), letter spacing (`T.letterSpacing.*`), radii (`T.radius.*`), spacing (`T.spacing.*`). Use `FONTS.*` from `src/theme/fonts.ts` for font families. Use `TIMING.*`, `MOTION.*`, `SPRING.*`, `EASE.*` for animations. Only hardcode values in truly unique one-off situations where no suitable token exists — if a new value is needed, add a token to `theme.ts` first. The only exceptions are: (1) domain colors in data files (`cards.ts`, `packs.ts`, `constants.ts`) where color is part of the data model, (2) stamina-specific cyan (`#4fc3f7`), and (3) colors with dynamic alpha that don't have a token (use `T.accent.mint + '66'` pattern).
